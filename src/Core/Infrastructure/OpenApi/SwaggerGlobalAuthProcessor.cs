...
public class SwaggerGlobalAuthProcessor : IOperationProcessor
{
    ...
    public bool Process(OperationProcessorContext context)
    {
        ...
        if (list is not null)
        {
            ...
            {
                ICollection<OpenApiSecurityRequirement> openApiSecurityRequirements = context.OperationDescription.Operation.Security ??= new List<OpenApiSecurityRequirement>();
                openApiSecurityRequirements.Add(new OpenApiSecurityRequirement
                {
                    {
                        _name,
                        Array.Empty<string>()
